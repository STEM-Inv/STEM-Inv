...
from free_u import register_upblock2d, register_free_upblock2d, register_crossattn_upblock2d, register_free_crossattn_upblock2d
...
class TokenFlow(nn.Module):
    ...
    def get_latents_path(self):
        latents_path = os.path.join(
            config["latents_path"], f'sd_{config["sd_version"]}',
    ...
    def batched_denoise_step(self, x, t, indices):
        ...
        register_pivotal(self, True) #key frame

        self.denoise_step(x[pivotal_idx], t, indices[pivotal_idx])
        
        register_pivotal(self, False) #non keyframe
    ...
    def init_method(self, conv_injection_t, qk_injection_t):
        ...
        #register_extended_stem_attention_pnp(self, self.qk_injection_timesteps)
    ...
    def edit_video(self):
        ...
        self.save_vae_recon()
        pnp_f_t = int(self.config["n_timesteps"] * self.config["pnp_f_t"]) #40=50*0.8
        pnp_attn_t = int(self.config["n_timesteps"] * self.config["pnp_attn_t"])#25=50*0.5
        ...
        save_video(edited_frames, f'{self.config["output_path"]}/tokenflow_PnP_fps_10.mp4', fps=10)
...
if __name__ == '__main__':
    ...
    if os.path.isfile(config["data_path"]):
        video_name = Path(config["data_path"]).stem
        config["data_path"] = f'data/{video_name}'
