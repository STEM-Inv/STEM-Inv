...
#################################################### STEM ####################################################
@torch.no_grad()
def initialize(num_base, k):
    """Base initialization for EM
    params: num_base, number of bases
    params: k, key features, b x n x c
    """
    std, mean = torch.std_mean(k, dim=1, keepdim=True)   # b, 1, c
    std = std.repeat(1, num_base, 1)
    mean = mean.repeat(1, num_base, 1)
    k_base = torch.normal(mean, std)
    return k_base

@torch.no_grad()
def e_step(k_base, k):
    """E Step for EM
    params: k_base, base features, b x m x c
    params: k, key features, b x n x c
    """
    tau = 0.01
    k_base_ = k_base / (torch.linalg.norm(k_base, dim=-1, keepdim=True) + 1e-4)
    #k_ = k / (torch.linalg.norm(k, dim=-1, keepdim=True) + 1e-4)
    z = torch.einsum("bmc,bnc->bnm", [k_base_, k])
    max_z = torch.max(z, dim=-1, keepdim=True)[0]
    z = torch.softmax((z - max_z)/tau, dim=-1)
    return z

@torch.no_grad()
def m_step(z, k):
    """M Step for EM
    params: z, responbilities, b x n x m
    params: k, key features, b x n x c
    """
    z = z / (torch.sum(z, dim=-2, keepdim=True) + 1e-4)
    k_base = torch.einsum("bnc,bnm->bmc",[k, z])
    return k_base, z

@torch.no_grad()
def em(k, v, num_bases=256, n_iters=5):
    """EM
    params: k, key features, b x n x c
    params: v, value features, b x n x c'
    params: num_bases, number of bases
    params: n_iters, number of EM interations
    """
    # init
    k_base = initialize(num_bases, k)
    ...
    v_base = torch.einsum("bnc,bnm->bmc", [v, z])

    return k_base.contiguous(), v_base.contiguous()
#################################################### STEM ####################################################
...
def register_extended_stem_attention_pnp(model, injection_schedule):
    def sa_forward(self):
        to_out = self.to_out
        if type(to_out) is torch.nn.modules.container.ModuleList:
            to_out = self.to_out[0]
        ...
        def forward(x, encoder_hidden_states=None, attention_mask=None):
            ...
            k_source = k[:n_frames].reshape(1, n_frames * sequence_length, -1)
            ...
            v_source = v[:n_frames].reshape(1, n_frames * sequence_length, -1)
            ...
            k_source, v_source = em(k_source, v_source)  # h, m, c//h
            k_uncond, v_uncond = em(k_uncond, v_uncond) 
            k_cond, v_cond = em(k_cond, v_cond)
            
            k_src = k_source.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)
            v_src = v_source.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)
            k_uncond = k_uncond.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)
            v_uncond = v_uncond.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)
            k_cond = k_cond.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)
            v_cond = v_cond.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)  
...
def register_extended_stem_attention(model):
    def sa_forward(self):
        to_out = self.to_out
        if type(to_out) is torch.nn.modules.container.ModuleList:
            to_out = self.to_out[0]
        ...
        def forward(x, encoder_hidden_states=None, attention_mask=None):
            ...
            k = k.reshape(1, n_frames, sequence_length, -1).flatten(start_dim=1, end_dim=2)  # 1, n, c
            v = v.reshape(1, n_frames, sequence_length, -1).flatten(start_dim=1, end_dim=2)  # 1, n, c'

            k = self.head_to_batch_dim(k)   # h, n, c//h
            v = self.head_to_batch_dim(v)

            k, v = em(k, v)  # h, m, c//h

            k = k.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)
            v = v.view(1, h, -1, dim // h).repeat(n_frames, 1, 1, 1)           
            q = self.head_to_batch_dim(q).view(n_frames, h, sequence_length, dim // h)
            
            out = []
            for j in range(h):
                sim = torch.bmm(q[:, j], k[:, j].transpose(-1, -2)) * self.scale
                out.append(torch.bmm(sim.softmax(dim=-1), v[:, j]))

            out = torch.cat(out, dim=0).view(h, n_frames, sequence_length, dim // h).permute(1, 0, 2, 3).reshape(h * n_frames, sequence_length, -1)
            out = self.batch_to_head_dim(out)
