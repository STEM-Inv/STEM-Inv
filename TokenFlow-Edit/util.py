...
def save_video_frames(video_path, img_size=(512, 512)):
    print(f"video path: {video_path}, is file = {os.path.isfile(video_path)}")
    if os.path.isfile(video_path):
        ...
        os.makedirs(f'data/{video_name}', exist_ok=True)
        video = sorted(os.listdir(video_path))
        for i in range(len(video)):
            ind = str(i).zfill(5)
            image = Image.open(os.path.join(video_path, video[i])).convert('RGB')
            image_resized = image.resize(img_size,  resample=Image.Resampling.LANCZOS)
            image_resized.save(f'data/{video_name}/{ind}.png')
    else:
        raise TypeError(f"type of video_path is not supported.")
