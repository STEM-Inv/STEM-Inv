...
class Preprocess(nn.Module):
    ...
    def init_method(self):
        register_extended_stem_attention(self)
...
def prep(opt):
    ...
    timesteps_to_save, num_inference_steps = get_timesteps(
        toy_scheduler, num_inference_steps=opt.save_steps,
...
if __name__ == "__main__":
    ...
    opt = edict(config)
    opt.save_dir = opt.latents_path
    opt.steps = opt.n_inversion_steps
    opt.save_steps = opt.n_timesteps

    save_video_frames(opt.data_path, img_size=(opt.width, opt.height))
    if os.path.isfile(opt.data_path):
        video_name = Path(opt.data_path).stem
        opt.data_path = f'data/{video_name}'
    prep(opt)
